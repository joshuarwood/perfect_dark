						...
						if (spdc[3] > 0.0f) {
							...
							if (g_ZbufPtr1
								...
								if (index < MAX_ARTIFACTS) {
									 ...
									 * Adding 0.5f ensures the subsequent conversion to an integer rounds
									 * to the nearest pixel (i.e. 0.2f becomes 0, 0.8f becomes 1).
									 */
									f32 xf = (viewleft + (1.0f + spdc[0] * f20) * viewwidth * 0.5f) * videoGetWidth() / videoGetNativeWidth() + 0.5f;
									f32 yf = (viewtop + (1.0f - spdc[1] * f20) * viewheight * 0.5f) * videoGetHeight() / videoGetNativeHeight() + 0.5f;
									/**
									 * note: we recompute xi, yi as floating point values xf, yf because
									 * trying to scale xi, yi directly results in rounding errors that
									 * select the wrong pixel.
									 */
									artifact->screenx = (u16)(xf > 0 ? xf : 0) % videoGetWidth();
									artifact->screeny = (u16)(yf > 0 ? yf : 0) % videoGetHeight();
